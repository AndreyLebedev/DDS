...
class database
{
  public:
    ...
    explicit database(const std::string& filename)
        : connected_(false)
    {
        if (sqlite3_open(filename.data(), &database_) == SQLITE_OK && sql("PRAGMA foreign_keys = ON;"))
        {
            connected_ = true;
        }
    }
    ...
    {
        return (sqlite3_exec(database_, query.data(), nullptr, nullptr, nullptr) == SQLITE_OK);
    }
    ...
    {
        sql_statement call;
        if (sqlite3_prepare_v2(database_, query.data(), -1, &call.statement, nullptr) != SQLITE_OK)
        {
            return false;
        }
    ...
    //            sql_statement call;
    //
    //            if (sqlite3_prepare_v2(database_, q, -1, &call.statement, tail) != SQLITE_OK)
    //            {
    //                return false;
    //            }
    //            if (!bind(call.statement, 1, args...))
    ...
    //                return false;
    //            }
    //            if (sqlite3_step(call.statement) != SQLITE_DONE)
    //                return false;
    //
    //            if (tail != nullptr)
    //                q = *tail;
    ...
    //            if (tail == nullptr)
    //                std::cout << "Tail is nullptr";
    //            else
    //                std::cout << "Tail is not null and equals to: " << tail;
    //
    //        } while (tail != nullptr);
    //        return true;
    //    }
     ...
     */
    template <typename... Args>
    bool sql(const std::string& query, std::vector<Row>& rows, const Args&... args)
    {
        sql_statement call;
        if (sqlite3_prepare_v2(database_, query.data(), -1, &call.statement, nullptr) != SQLITE_OK)
        {
  ...
  private:
    ...
    static bool bind(sqlite3_stmt*, const int)
    {
        return true;
    }
    ...
    struct bind_struct
    {
        static bool f(sqlite3_stmt* statement, int current, const T& first, const Args&... args)
        {
            std::stringstream ss;
            ss << first;
            if (sqlite3_bind_text(statement, current, ss.str().data(), ss.str().length(), SQLITE_TRANSIENT) !=
                SQLITE_OK)
            {
                return false;
            }
    ...
    {
        static bool f(sqlite3_stmt* statement, int current, double first, const Args&... args)
        {
            if (sqlite3_bind_double(statement, current, first) != SQLITE_OK)
            {
                return false;
            }
            return bind(statement, current + 1, args...);
    ...
    {
        static bool f(sqlite3_stmt* statement, int current, int first, const Args&... args)
        {
            if (sqlite3_bind_int(statement, current, first) != SQLITE_OK)
            {
                return false;
            }
            return bind(statement, current + 1, args...);
    ...
    {
        static bool f(sqlite3_stmt* statement, int current, const std::vector<char>& first, const Args&... args)
        {
            if (sqlite3_bind_blob(statement, current, &first[0], first.size(), SQLITE_TRANSIENT) != SQLITE_OK)
            {
                return false;
            }
            return bind(statement, current + 1, args...);
